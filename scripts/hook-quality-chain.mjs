...
// with the orphan sweep ENABLED between attempts: the orphaned node.exe left
// by a watchdog timeout is exactly what keeps the panic alive, so sweeping it
// between attempts clears the panic and the commit succeeds.
//
...
/**
 * Run the quality chain with panic retry + orphan sweep between attempts.
 * Resolves with the chain's exit code (0 on success); never rejects.
 */
