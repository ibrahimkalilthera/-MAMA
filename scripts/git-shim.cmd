...
rem so it stays alive exactly when the panic is active. It routes every
rem `git commit` / `git push` through the retry wrapper of the CURRENT repo
rem (scripts/git-retry.mjs --sweep): attempt 1 dies if the hook's sh is hit by
rem the panic, the sweep kills the orphaned node.exe that keeps the panic
rem alive, attempt 2 succeeds. Every other git command is forwarded untouched
rem to the real git.exe.
rem
...
rem Only commit/push need the retry wrapper; everything else goes straight
rem to the real git.
...
rem forward (other repos are unaffected by this shim).
