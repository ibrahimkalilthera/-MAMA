...
**La preuve ne voyage plus dans un journal — elle est STOCKÉE.** Le transport a été remplacé le jour où ses trois défauts ont été nommés : le runner réécrit ce qui traverse un journal (le titre d'une annotation disparaît, un contrat posé là a été vert, testé, et aveugle sur le run qu'il visait), n'importe quoi qui **imprime** une marque se fait passer pour l'automatisation qui l'émet, et GitHub n'archive un journal qu'**après** avoir marqué le run terminé (d'où des `logs 404` sur des runs verts qui venaient de finir, et toute une mécanique de fenêtre de grâce). Chaque automatisation publie donc sa preuve par une étape **nommée** (`Publier la preuve d’action` → `scripts/publish-automation-evidence.mjs`), que le runner stocke en **champs structurés** (titre + message d'annotation) ; `npm run check:automations` lit ces champs par l'API (`checks: read`) et ne télécharge **aucun** journal. Le producteur ne peut pas mentir sur son sujet : le nom du workflow lui est **imposé** par `GITHUB_WORKFLOW_REF`, et il refuse une raison vide, `--acted` avec `--count 0`, ou les deux états à la fois. Un run qui avait l'étape de preuve et n'a rien publié est un **faux vert** (`unproven`) ; un run **antérieur** au contrat est `legacy` — nommé, non bloquant, jugé sur sa seule conclusion — parce que l'audit lit les **étapes** du run pour savoir si une preuve était attendue, au lieu de deviner une fenêtre de tolérance.

**Un contrôle qui lit des preuves doit se méfier de sa propre suite de tests.** C'est exactement ce qui a coûté le 2026-09-12 : l'audit cherchait un mot dans des journaux, or `npm test` exécute des suites *qui parlent de ces mots-là*. Le 2026-09-12 il a donc accusé `Quality & performance guard` avec le motif de `Dependabot rebase` : la marque avait été **imprimée** dans le journal du job de tests par le script Dependabot lui-même (`main()` tournait à l'import), et l'audit lisait ce journal. Trois protections le rendent impossible : la preuve **porte son sujet** (`AUTOMATION-EVIDENCE {"workflow":"…"}`, comptée seulement si elle nomme le workflow jugé, et l'emprunt est **cité** quand il ne correspond pas) ; **importer un module ne travaille pas** (`npm run check:import-effects` refuse tout module importé par une suite qui écrit dehors à l'import) ; et **un marqueur ne s'imprime que par son automatisation** (`npm run check:gate-sentinels` : 155 fichiers scannés, une liste blanche d'émetteurs, et les commentaires blanchis — un commentaire ne s'exécute pas).
