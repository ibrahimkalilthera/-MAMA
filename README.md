...
**La preuve est faite sur le binaire livré, pas seulement en unité** : `node scripts/verify-updater.mjs` sert un flux local à l'exe empaqueté. Avec `UPDATER_FAKE_VERSION=2.0.0` il exige que l'exe **annonce l'obligation avec son motif** (`update-available 2.0.0 — OBLIGATOIRE (1 version(s) majeure(s) de retard)`) ; à majeure égale, il exige l'inverse — une obligation annoncée pour un correctif bloquerait une école entière pour rien, donc le faux positif est testé aussi. Les deux passes exigent un **téléchargement réel** (`download-progress`) et **au moins deux vérifications dans la même session** : mesuré, `check focus` ouvre le bal, `check startup` est refusé comme trop rapproché, puis l'intervalle redéclenche.
