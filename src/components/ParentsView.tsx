...
export type ParentsViewProps = Pick<MainViewsProps, 'AlertCircle' | 'ArrowUpDown' | 'Bell' | 'Briefcase' | 'CheckCircle2' | 'ChevronDown' | 'ChevronRight' | 'ChevronUp' | 'CreditCard' | 'Download' | 'Edit2' | 'FileText' | 'HighlightText' | 'Mail' | 'MapPin' | 'Phone' | 'Plus' | 'Receipt' | 'Search' | 'Trash2' | 'Unlink' | 'UserCheck' | 'UserPlus' | 'Users' | 'X' | 'currentTheme' | 'expandedParentId' | 'formatCurrency' | 'getChildrenForParent' | 'getParentOutstandingBalance' | 'getParentPaymentHistory' | 'handleDeleteParent' | 'handleExportParentLedgerPdf' | 'handleUnlinkStudent' | 'openEditParentModal' | 'openNotifyModal' | 'parentChildrenSortBy' | 'parentSearchTerm' | 'parents' | 'selectedYear' | 'setActiveLinkingParent' | 'setEditingParent' | 'setExpandedParentId' | 'setParentChildrenSortBy' | 'setParentForm' | 'setParentSearchTerm' | 'setSelectedStudent' | 'setShowLinkStudentModal' | 'setShowParentModal' | 'setStudentToLinkId' | 't'>;

export function ParentsView(props: ParentsViewProps) {
  const { AlertCircle, ArrowUpDown, Bell, Briefcase, CheckCircle2, ChevronDown, ChevronRight, ChevronUp, CreditCard, Download, Edit2, FileText, HighlightText, Mail, MapPin, Phone, Plus, Receipt, Search, Trash2, Unlink, UserCheck, UserPlus, Users, X, currentTheme, expandedParentId, formatCurrency, getChildrenForParent, getParentOutstandingBalance, getParentPaymentHistory, handleDeleteParent, handleExportParentLedgerPdf, handleUnlinkStudent, openEditParentModal, openNotifyModal, parentChildrenSortBy, parentSearchTerm, parents, selectedYear, setActiveLinkingParent, setEditingParent, setExpandedParentId, setParentChildrenSortBy, setParentForm, setParentSearchTerm, setSelectedStudent, setShowLinkStudentModal, setShowParentModal, setStudentToLinkId, t } = props;
  return (
                ...
                parents.length === 0 ? (
                  <div className={`p-12 text-center rounded-[2rem] ${currentTheme.card} border ${currentTheme.border} space-y-4`}>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className={`text-xs font-semibold ${currentTheme.muted} max-w-[300px] mx-auto`}>{t.noParentsInSystemHint}</p>
                    </div>
                    <button
                      onClick={() => {
                        setEditingParent(null);
                      ...
                      className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-emerald-600 text-white font-bold text-xs hover:bg-emerald-700 shadow-lg shadow-emerald-600/20 transition-all"
                    >
                      <Plus size={16} />
                      <span>{t.addParent}</span>
                    </button>
