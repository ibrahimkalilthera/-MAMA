 ...
 *
 * Single source of truth for the data-model shapes used by BOTH the UI layer
 * (historically src/app/types.ts) and the data layer (historically
 * src/lib/useSupabaseData.ts, which used to re-declare identical interfaces —
 * see DEVELOPMENT_HISTORY.md). src/app/types.ts and src/lib/useSupabaseData.ts
 * now re-export from here, so every import site keeps working unchanged.
 *
 * Pure module: no imports, no runtime logic beyond DEFAULT_SCHOOL_CLASSES.
 */
...
/** A custom (user-created) class row from the database, as opposed to the
 * built-in DEFAULT_SCHOOL_CLASSES above. */
