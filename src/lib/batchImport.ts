/**
 * Smart Excel Ingestion batch import.
 *
 * Extracted from useSupabaseData.ts: the per-category insert/update logic was a
 * self-contained 270-line closure inside the data hook, so it now lives here as
 * a pure function over a deps bag (state arrays + fetch/notify callbacks).
 * Behaviour is unchanged — tests/offline-sync + the Excel import suites cover it.
 */
...
/** Everything the import loop needs from the owning hook. */
export interface BatchImportDeps {
  students: Student[];
  parents: Parent[];
  staff: Staff[];
  expenses: Expense[];
  /** Refresh in-memory data after a successful import. */
