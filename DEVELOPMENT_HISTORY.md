...
- **Floating chat extracted (2026-08-31)**: App.tsx lost ~300 lines — the floating widget (panel + FAB) moved to [`FloatingChat.tsx`](src/components/FloatingChat.tsx) and both AI surfaces (Productivité AI tab + floating chat) moved verbatim into the `useFloatingChat` domain hook ([`src/app/useFloatingChat.ts`](src/app/useFloatingChat.ts)); two new translation keys (`floatingChatTitle`/`floatingChatPlaceholder`) land in both dictionaries for the panel header/input.
