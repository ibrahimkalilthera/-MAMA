...
- **Vérifié en vrai** : audit `AUDIT_THEMES=slate,midnight` → « Fiche Élève — 32 textes scannés » **dans les deux thèmes** (aucune autre étape ne change de volume), **36/36 couvertures, 0 paire sous 3:1** ; `npm run lint` → exit 0 (sélecteurs CSS, stylelint, budget de lignes compris) ; **752/752** tests.
