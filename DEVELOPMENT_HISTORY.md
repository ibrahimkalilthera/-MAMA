...
- **Deploy workflow is currently failing — action required**: [`deploy.yml`](.github/workflows/deploy.yml) has **never succeeded** (5/5 runs red since it was introduced). Everything through `npm ci --prefix tools` passes; it fails at `vercel pull --environment=production`, which means the repository secrets are missing or invalid — `VERCEL_TOKEN` (and check `VERCEL_ORG_ID`/`VERCEL_PROJECT_ID`, values documented above) in *Settings ▸ Secrets and variables ▸ Actions*. No `vercel` app check-run appears on recent commits, so no Vercel git-integration deploy is compensating either: **recent pushes have not reached production**. Until the secret is added, the deploy steps after the audit cannot run. Triggers, for reference: both workflows fire on `push` to `main` (deploy also on manual `workflow_dispatch`); `perf-guard` uses concurrency group `quality-*` with cancel-in-progress, `deploy` uses `deploy-*` without.
