## [2026-09-11] CI rouge sur ubuntu : suites Windows-only neutralisées — la porte qualité (et le déploiement Vercel) repasse au vert

Depuis `217f242`, le job `quality` de `perf-guard.yml` échouait sur `ubuntu-latest` à l'étape **Tests** : `deploy.yml` gate le déploiement sur le succès de ce workflow, donc **plus aucun déploiement Vercel** (« Quality workflow concluded 'failure' on e614059… — nothing to deploy »). Cause : les suites ajoutées par les chantiers sweep / garde / hooks assertaient un comportement **Windows-only** sans injecter la plateforme — sur un runner Linux elles étaient soit des no-op (`orphan-chrome.mjs` lit `process.platform` en interne), soit impossibles (`git-shim.test.ts` lance un vrai `cmd.exe`).
...
- **Vérification** : la CI ubuntu est **rejouée localement** avec un preload qui force `process.platform = 'linux'` (`node --import`) — 7 échecs reproduits avant correctif, **62/62 verts après** (4 tests shim skippés) sur les 7 suites concernées ; 66/66 sur Windows. tsc 0 erreur, lint vert.
