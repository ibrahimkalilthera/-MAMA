...
- **Vérifié** : 16 tests (tri et parts, delta dont un maillon sans historique, gain vs plancher, plan absent sous deux maillons, exclusion de `tests`/`build`, formatage, cache, câblage de la mesure d'hygiène) ; exécution réelle deux fois de suite → `audit-gate 10.6 s (88%)` puis `5.5 s (78%) −5.2 s`, avec `sweep:chrome`/`sweep:electron`/`purge:exit` visibles ; `npm run lint` et suite complète.
