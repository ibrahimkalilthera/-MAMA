...
**Le problème est qu'un dossier n'a ni numéro ni empreinte.** `release/win-unpacked` pèse 507 Mo (67 % de l'atelier) et le canal ne peut donc rien dire de lui : la règle de l'atelier ne condamne que sur un digest d'octets publiés, donc le plan le nommait, le pesait, et laissait la décision à un humain. Le dernier volume sans preuve était aussi le plus gros.

**La preuve existe pourtant, et elle ne demande aucun téléchargement : un manifeste est une fonction déterministe de l'arbre.** `scripts/lib/unpacked-manifest.mjs` décrit chaque fichier (chemin, taille, sha256), **trie** les entrées et **normalise** les séparateurs, puis hache cette description canonique. `npm run manifest:unpacked` (`write-unpacked-manifest.mjs`), appelé à la fin de `electron:build` — donc jamais à la main — écrit `MamaTheraFinance-<version>-unpacked.manifest.json`, et ce fichier **voyage avec le lot** : le publieur le téléverse à côté de l'installeur (`assetsToPublish`), et le contrôle de cohérence exige qu'un release le porte (`expectedArtifacts`). Côté atelier, le même module recompose le manifeste de l'arbre local et compare les **empreintes** : égales ⇒ le canal sert déjà ces octets-là, fichier par fichier, donc le dossier part sur le seul `--yes` (famille `digest`, exactement comme un fichier dont le digest correspond).

**Un parcours, pas deux.** Le parcours d'arborescence (`hashTree`) vit dans le module, et le build comme l'atelier l'appellent : deux parcours séparés finiraient par ne plus décrire la même chose (un séparateur, un type d'entrée), et la comparaison deviendrait un faux négatif permanent — 507 Mo conservés en croyant qu'aucune preuve n'existe.

**Quatre lectures, quatre sorts, et chacune est écrite ligne par ligne** dans le bloc `🔐 arborescences` : un manifeste **publié d'une autre version** condamne ; celui de la **version en cours** ne condamne pas (son numéro est en préparation, donc c'est la sortie de build que les preuves locales lisent — même règle que pour les fichiers, qui ne sont pas non plus hachés) ; un manifeste qui **ne décrit pas** cet arbre ne condamne rien (« jamais condamnée sur un doute ») ; un dossier **illisible** (fichier verrouillé par l'application qui tourne) non plus. Et la convention `-unpacked` reste **seule** à décider ce qui est une sortie de build : une preuve ne peut pas faire partir les ressources de build (`.icon-ico` et compagnie), même si un manifeste portait leur empreinte — un cas de test le verrouille.

**Le coût est mesuré, et il n'est pas caché.** Hacher les 508 Mo réels prend **5,9 s** (76 fichiers) : c'est trop pour un hook de commit, et inutile quand rien ne peut être condamné. Trois décisions en découlent, toutes dites dans le rapport : (1) l'atelier ne hache que si un manifeste **d'une autre version** est publié — sinon le verdict ne pourrait pas changer, et c'est écrit plutôt que payé en silence ; (2) si seul celui de la version en cours est publié, même chose, nommément ; (3) le hook de commit et celui de push posent `WORKSHOP_TREE_PROOF=0` et le rapport dit « preuve d'arborescence non tentée (WORKSHOP_TREE_PROOF=0) : hacher ce dossier ralentirait chaque commit — la CI la lance, elle, strictement ». Le **job de publication** ne connaît aucune dispense, et un cas de test l'exige (il refuse aussi bien un `WORKSHOP_SOFT_OFFLINE` qu'un `WORKSHOP_TREE_PROOF` dans ce workflow).

**Ce qui n'est PAS prétendu** : aucune arborescence n'est encore prouvable en vrai sur ce poste, parce que le canal ne publie de manifeste pour **aucune** version — le premier release qui en portera un ouvrira la preuve, et d'ici là la ligne dit exactement cela. Le manifeste local du build en cours n'est pas écrit à la main non plus : c'est `electron:build` qui le fait, et un manifeste écrit à la main décrirait un arbre d'avant-hier.

**Mesures** : `tests/unpacked-manifest.test.ts` (**13** cas, dont un oracle indépendant : l'empreinte d'un arbre identique écrit à deux endroits différents, et un octet changé qui la change) ; `release-prune.test.ts` **38/38** (+4 : condamnation par preuve, absence de preuve, non-sortie-de-build protégée, manifeste versionné comme un artefact) ; `prune-cli.test.ts` **24/24** (+6 sur le CLI lancé pour de vrai, canal lu depuis un fixture, dont la dispense du hook déclarée et le manifeste écrit par le build). `tsc --noEmit` propre.
...
**Le premier trou était celui de la demande d'origine : rien ne lançait jamais la purge.** `release:prune` n'existait que dans `package.json` — aucun workflow, aucun hook — et `check:release` **nommait** bien les installeurs étrangers mais sortait en **0**. Donc le jour où personne ne se souvient de la commande, le dossier regrossit pendant qu'un contrôle vert regarde ailleurs : exactement l'état initial. Le jugement est désormais un **maillon de la chaîne qualité** (`npm run check:release:workshop`, soit `prune-release-dir.mjs --check`, qui sort en échec dès qu'un fichier est **prouvé** redondant), et il suit le précédent du gate d'audit : **hors ligne il se tait en le DISANT** (`WORKSHOP_SOFT_OFFLINE=1`, posé par pre-commit **et** pre-push), alors que la CI le lance strict. La calibration compte autant que le maillon : mesuré sur le dossier réel, il est **vert** (le trio 1.0.6 est la sortie du build en cours, les contrôles locaux la lisent) et **rouge** dès qu'un octet est prouvé servi par le canal — un gate qui rougit sur du travail légitime serait désactivé dans la semaine. Les deux frontières sont dites, pas devinées : un dossier **absent** rend `✅ atelier non applicable — release/ n'existe pas ici (rien à juger)` — un vert qui s'explique, jamais un vert muet — et une **sortie de build** (`win-unpacked`, 507 Mo) n'est pas condamnable par une empreinte (aucun fichier livrable à comparer) : elle est **nommée avec son poids** et part par un acte **structurel** distinct (`--yes --unpacked`) — pour que les 508 Mo cessent d'être invisibles à la règle et à l'œil.

**Le deuxième était le seul où être serviable est dangereux.** Mesuré en direct : le plan d'un `release-probe/` annonçait `Applique-le : npm run release:prune -- --yes --stale --unpublished`, c'est-à-dire la commande qui purge le **vrai** `release/`. `pruneCommand(plan)` est maintenant le **seul** propriétaire de cette phrase — il déduit les drapeaux **et** `--dir` du plan — donc les trois littéraux (dont un seul avait reçu le correctif) sont devenus un, et recopier le rappel agit sur le dossier qu'on vient de lire.

**Le troisième était structurel, et il cachait le précédent.** `scripts/lib/release-coherence.mjs` portait **cinq métiers** dans 727 lignes (lire le flux, comparer local↔distant, juger la portée du canal, lire le frein, décider une publication) : rien n'y était remplaçable séparément. Découpé en `latest-yml.mjs`, `release-compare.mjs`, `release-reach.mjs`, `release-version.mjs` — les 135 cas passent **sans être réécrits**, et le lien d'immunité déclaré a suivi le module qui porte la preuve. Au passage, `compareVersions` existait **deux fois** avec deux sens : l'un rend `null` sur une version illisible, l'autre (`check-vercel-pins.mjs`) la lisait `0` — deux réponses à « A est-elle plus récente que B » ; il n'en reste qu'une, importée. Et le filtre de chemins de `release-channel-watch.yml` nommait le fichier supprimé : le contrôle n'aurait plus été déclenché par le push qui le change — corrigé vers les quatre modules.

**Deux défauts que j'ai fabriqués, et qui valent d'être notés parce qu'ils sont la forme exacte que ce dépôt traque.** (1) Le découpage a laissé chaque bloc de documentation au-dessus de la **mauvaise** déclaration : tsc en a tiré `deliveryReach → number[]` et `publishDecision → ReleaseLike[]`, **61 erreurs** — un type faux qui ne casse rien à l'exécution, donc un mensonge silencieux du seul contrôle qui lit les contrats. Réassignés un par un, `tsc --noEmit` revient propre. (2) Ajouter le maillon `workshop` a invalidé le vert enregistré (le cache couvre un **ensemble** de maillons) : une suite qui devait être instantanée a lancé la **vraie** chaîne pendant **75 s**, puis a échoué sur son propre garde-fou (`écrire un cache sur un saut serait un abus`). Le fixture dérive maintenant ses maillons de `DEFAULT_STEPS`, donc il ne peut plus se désynchroniser du hook — et un cas neuf verrouille le comportement réel : **un vert enregistré AVANT qu'un maillon n'entre dans la chaîne doit rejouer**, parce qu'un vert réputé bon pour tout est précisément ce qu'on refuse.

**Et les vues se prouvent en se rendant, plus en se relisant.** Le regroupement par incident et l'export CSV « qui suit la vue » étaient tenus par cinq assertions par **expression régulière** sur le source d'`AuditView.tsx` (`useState(true)`, `incident.reports.map`, `viewRows.map((row) => row.kind === 'incident'`) : elles restaient vertes si la vue n'affichait plus rien. `tests/views-render.test.tsx` monte maintenant la vue avec de **vrais** rapports produits par le module de remontée et lit ce qui sort : deux postes bloqués par la même panne → **une** ligne (le compte et les postes nommés), les remontées brutes **repliées** puis **dépliées** (`aria-expanded` bascule, le chemin du journal local apparaît), et le CSV **suit la vue** — regroupé, en-tête + **une** ligne par incident ; dégroupé, en-tête + **une par poste**.

**Mesures** : `tsc --noEmit` propre ; `tests/views-render.test.tsx` 18/18, `blocked-incidents` 13/13, `prune-cli` 14/14 (des **lancements réels du CLI**, sans réseau, à la place des assertions-sur-source), `release-prune` 34/34, `release-coherence` 135/135, les deux suites du hook vertes **et de nouveau instantanées**. Le contrôle du maillon est vert sur le dossier réel, et rouge sur un fichier prouvé redondant (mesuré la veille : `1 digest détecté · exit 1`).
...
**La relance est partie, et c'est elle qui a mis le doigt sur le vrai défaut.** Deux commits poussés (`06d02ea`, `fafdb7c`), chaîne qualité verte avant chacun (1 min 22 s et 1 min 21 s), puis le push a déclenché le workflow du canal — qui est passé au **rouge**, sur son propre chemin (`.github/workflows/release-channel-watch.yml` est dans son filtre).

**Le rouge accusait le canal, et c'était faux.** `latest.yml absent du release « v1.0.6 » — un poste sans ce fichier ne voit rien`. Le fichier était là : quatre actifs sur le release, `latest.yml` compris, et la même commande relancée sur le poste de travail était **verte**. Ce qui échouait, c'est la **lecture** : `readAsset` allait chercher les octets à l'endpoint d'actif de l'API (`api.github.com/…/releases/assets/{id}`), dont les requêtes **anonymes** sont limitées **par adresse IP** — et un runner GitHub partage la sienne. L'API anonyme répondait donc autre chose qu'un `200`, le texte restait `null`, et le verdict concluait « absent ». Le statut HTTP, lui, était **jeté** (`if (!res.ok) return { status: res.status }`) : le script savait ce qu'il avait reçu et refusait de le dire.

**Deux corrections, et chacune se justifie seule.** D'abord la voie de lecture : le poste ne télécharge pas par l'API, il suit `browser_download_url` (`github.com/…/releases/download` → CDN), donc le contrôle fait pareil — c'est la voie réellement empruntée, et elle n'est pas soumise au quota de l'API. La seconde voie reste un repli, avec le statut de la première conservé. Ensuite le diagnostic : un actif **absent** de la liste et un actif **présent mais illisible** sont deux pannes qui se réparent différemment, donc elles ne portent plus le même mot — `latest.yml du release « v1.0.6 » n'a pas pu être LU (endpoint d'actif de l'API HTTP 403, voie du poste HTTP 403) — le canal n'est pas jugé tant que la lecture échoue`. Même traitement pour l'installeur (« ses octets n'ont pas pu être LUS » au lieu d'« introuvable parmi les artefacts »). Et une lecture qui a dû passer par la seconde voie **se dit** : un repli silencieux serait un vert qu'on ne saurait pas expliquer.

**État au moment d'écrire.** Les deux commits sont sur `origin/main` (`0 0`), le canal est vivant sans jeton — tête lue `v1.0.6`, les six versions publiées rattachées, frein lisible à 0 retenue. Et `Release channel watch` est désormais relancé **par la publication elle-même** (`release: [published]` pour une publication hors chaîne, `workflow_run` sur le publieur pour la publication automatique, que `release` ne couvre pas puisque GitHub ne cascade aucun événement produit par `github.token`) : la preuve attendue est un run déclenché par une publication réelle, et non plus par le seul cron.

## [2026-09-13] `release/` devient un atelier : ce qui est prouvé redondant part, ce qui ne l'est pas reste et se nomme

Demande : « Décide du sort du dossier release/ : huit installeurs d'anciennes versions y traînent et rien n'empêche d'y reprendre le mauvais fichier à la main. »

**Le dossier n'était pas « sale » : il était sans règle.** Sept versions, 2,2 Go, vingt et un fichiers au même niveau — dont quatorze installeurs — et pour seul signal le nom du fichier. Le contrôle local les **nommait** (un avertissement de `check:release`, exactement là où l'on prend le mauvais fichier) — un avertissement qu'on lit tous les jours ne se lit plus.

**La règle ne peut pas être « c'est vieux ».** Une version ancienne peut n'avoir **jamais** été publiée, et sa copie locale est alors la seule au monde : une suppression doit être une **preuve**. Un artefact local ne disparaît donc que si le canal déclare un actif du **même nom, de la même taille et de la même empreinte** — le `digest` sha256 que l'API GitHub expose par actif, c'est-à-dire des octets **déjà comptés sur le disque de GitHub**. Le plan ne télécharge rien (métadonnées seules) et hache le local, ce qui coûte un disque, pas un réseau. Quatre refus : un **brouillon** ne peut rien autoriser (invisible pour un poste, donc il ne « détient » rien là où un poste lit), une version **absente du canal** garde sa copie, une **empreinte absente** n'est pas une empreinte qui correspond (une taille seule n'identifie pas des octets), et un actif **non déclaré** reste.
...
**Un build non signé ne peut plus passer pour une publication.** Le job ne démarre que si le gate a ouvert — donc parce qu'il y avait un release à faire. Dans ce cas, « aucun certificat » veut dire « rien n'a été livré », et la preuve le dit (`--inert`, run rouge) au lieu de décrire des artefacts que personne ne recevra : c'est la même classe de faux vert que le reste de ce dépôt traque, et elle devenait atteignable dès que le déclencheur est devenu automatique. Mesure du 2026-09-13 : les secrets du dépôt sont `DEPENDABOT_REBASE_TOKEN, SUPABASE_SERVICE_ROLE_KEY, VERCEL_ORG_ID, VERCEL_PROJECT_ID, VERCEL_TOKEN` — `CSC_PFX_B64` n'y est pas, donc la chaîne automatique **construira sans publier** tant que le certificat n'est pas posé. C'est le dernier réglage, et c'est un secret, pas du code.
...
**Le workflow est planifié, et il publie sa mesure.** `.github/workflows/release-channel-watch.yml` : cron quotidien (un canal se casse **sans commit** — release supprimé à la main, fichier écrasé côté dépôt), `workflow_dispatch`, un push filtré sur le contrôle lui-même, `updates/holds.json` et l'inventaire — et, depuis la mesure du 13/09, **la publication** : le commit qui a publié la 1.0.6 n'a touché aucun des quatre chemins filtrés, donc le workflow n'a pas tourné et la CI n'a rien su avant le cron de 05:40, alors que la publication est précisément l'événement qui déplace la tête. `release: [published]` couvre la publication hors chaîne (un PAT à la main, comme celle-là) et `workflow_run` sur le publieur couvre la publication automatique — la seule des deux que `release` ne peut pas voir, GitHub ne cascadant aucun événement produit par `github.token`. Pas de `npm ci` : le script n'utilise que la bibliothèque standard, donc rien ne peut échouer avant la mesure. Le compte publié est le nombre d'artefacts vérifiés sur le release, et la raison nomme le tag et l'état du frein.
