...
- **Vérifié, en vrai** : sur ce poste (win32), le layout `node_modules` est trouvé à sa place et exécute npm 11.19.0 ; le même script lancé sous le runtime épinglé (qui n'embarque pas npm) rend la branche « non applicable » sans mentir sur la source (`path`) ; et le verdict **mord** sur les données réelles — contrat `linux` appliqué à cette machine → rouge, et un override pointant ailleurs que le npm de la machine → rouge avec le chemin attendu ; 25 tests (dont la vérification de la machine qui exécute la suite) ; `npm run lint` et suite complète.
