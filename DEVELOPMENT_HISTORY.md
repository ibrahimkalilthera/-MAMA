...
**Vérifié** : 13 cas neufs sur la politique (les trois seuils, la frontière de chaque seuil, une date dans le futur, une version illisible, une date illisible, un poste à jour ou en avance, et l'écart de mineures entre deux majeures qui n'est pas « une mineure »), le report ignoré quand c'est obligatoire, plus le câblage — le retard calculé côté main, l'absence de « Plus tard » et de `cancelId` en obligatoire, la porte à l'écran, la secours rendue uniquement sous `failed`, la relance nommée dans le pont, et la parité fr/en des huit libellés. **1122/1122** tests, `npm run lint` exit 0. `electron:dist` reconstruit l'interface avant d'empaqueter, donc la porte part avec la prochaine version publiée ; `scripts/verify-updater.mjs` reste à rejouer après un réempaquetage, comme pour la cadence.
